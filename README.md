...
## 提供形態
- Webダッシュボード: https://rv.insas.jp （公開URL、想定利用経路）
...
**手順書:** [docs/deploy-vps-ubuntu-apache.md](docs/deploy-vps-ubuntu-apache.md)  
**本番 rv.insas.jp（/home/rv/www）:** [docs/deploy-rv-production.md](docs/deploy-rv-production.md)  
**GitHub 自動デプロイ補足:** [docs/deploy-vps-github-続き.md](docs/deploy-vps-github-続き.md)

| ファイル | 内容 |
|---------|------|
| `web/deploy/researchvault.service` | systemd ユニット |
| `web/deploy/apache-rv.insas.jp.conf` | Apache リバースプロキシ |
| `web/deploy/env.production.example` | 本番 `.env` テンプレート |
| `web/deploy/deploy.sh` | `git pull` → build → restart |

最小構成: Node で `npm run build && npm run start`（ポート 3000）→ Apache が `127.0.0.1:3000` へプロキシ。
